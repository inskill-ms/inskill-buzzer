...
function inskill_buzzer_get_top3_score_mode2() {
    ...
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT participant_name FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d AND buzz_time IS NOT NULL ORDER BY buzz_time ASC LIMIT 3",
        $activity_id
    ), ARRAY_A );
...
/**
 * Met à jour le score du participant ayant buzzé en premier pour le mode 2.
 */
function inskill_buzzer_update_first_score_mode2() {
    ...
    $participant = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, participant_name, score FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d AND buzz_time IS NOT NULL ORDER BY buzz_time ASC LIMIT 1",
        $activity_id
    ), ARRAY_A );
    if (!$participant) {
    ...
    $updated = $wpdb->query(
        $wpdb->prepare(
            "UPDATE {$wpdb->prefix}inskill_buzzer_mode2_scores SET score = score + %d WHERE id = %d",
            $delta,
            $participant['id']
        )
    );
...
/**
 * Récupère le classement complet pour le mode 2.
 */
function inskill_buzzer_get_full_ranking_mode2() {
    ...
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT participant_name, score FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d ORDER BY score DESC, CASE WHEN buzz_time IS NULL THEN 1 ELSE 0 END, buzz_time ASC",
        $activity_id
    ), ARRAY_A );
    wp_send_json_success( $results );
}
...
function inskill_buzzer_update_participant_score_mode2() {
    ...
    $result = $wpdb->query(
         $wpdb->prepare(
             "UPDATE {$wpdb->prefix}inskill_buzzer_mode2_scores SET score = score + %d WHERE activity_id = %d AND participant_name = %s",
