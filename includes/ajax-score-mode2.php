...
/**
 * Met à jour le score de l'équipe du participant ayant buzzé en premier pour le mode 2.
 */
function inskill_buzzer_update_first_score_mode2() {
    global $wpdb;
    $activity_id = intval(get_option('inskill_buzzer_mode2_activity_id', 0));
    ...
    // Sélectionner le premier buzz pour obtenir l'équipe
    $participant = $wpdb->get_row( $wpdb->prepare(
        "SELECT team FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d AND buzz_time IS NOT NULL ORDER BY buzz_time ASC LIMIT 1",
        $activity_id
    ), ARRAY_A );
    if (!$participant) {
        wp_send_json_error( array( 'message' => 'Aucun participant buzzé' ) );
    }
    // Mettre à jour le score pour tous les participants de cette équipe
    $updated = $wpdb->query(
        $wpdb->prepare(
            "UPDATE {$wpdb->prefix}inskill_buzzer_mode2_scores SET score = score + %d WHERE activity_id = %d AND team = %s",
            $delta,
            $activity_id,
            $participant['team']
        )
...
add_action('wp_ajax_update_first_score_mode2', 'inskill_buzzer_update_first_score_mode2');
add_action('wp_ajax_nopriv_update_first_score_mode2', 'inskill_buzzer_update_first_score_mode2');

/**
 * Récupère le classement complet pour le mode 2 (agrégé par équipe) en calculant la moyenne sans décimale.
 */
function inskill_buzzer_get_full_ranking_mode2() {
    ...
    // Calcul de la moyenne arrondie sans décimale
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT team, ROUND(SUM(score)/COUNT(*)) as score FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d AND team != '' GROUP BY team ORDER BY score DESC",
        $activity_id
    ), ARRAY_A );
    wp_send_json_success( $results );
}
...
/**
 * Met à jour le score d'un participant pour le mode 2.
 * Modification : si un paramètre "team" est fourni, il est utilisé directement.
 */
...
/**
 * Nouvel endpoint : Récupérer la liste des participants connectés pour le mode 2.
 */
function inskill_buzzer_get_connected_participants_mode2() {
    global $wpdb;
    $activity_id = intval(get_option('inskill_buzzer_mode2_activity_id', 0));
    if($activity_id === 0){
         wp_send_json_error(array('message' => 'Activité non définie'));
    ...
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT participant_name, team FROM {$wpdb->prefix}inskill_buzzer_mode2_scores WHERE activity_id = %d ORDER BY team ASC, participant_name ASC",
        $activity_id
    ), ARRAY_A );
    wp_send_json_success( $results );
}
add_action('wp_ajax_get_connected_participants_mode2', 'inskill_buzzer_get_connected_participants_mode2');
add_action('wp_ajax_nopriv_get_connected_participants_mode2', 'inskill_buzzer_get_connected_participants_mode2');
?>
