...
class InskillBuzzer_Shortcodes_Mode1 extends InskillBuzzer_Shortcodes {
    ...
    public function render_mode1_animateur($atts) {
        ...
        /* Ajout d'un espacement entre les lignes des boutons Bonus/Malus */
        .result-row {
            margin-bottom: 20px;
        }
        table.ranking-table {
        ...
        /* Espacement supplémentaire entre le bloc "No good !" et le titre "Classement" */
        .classement-heading {
            margin-top: 20px;
        }
        </style>
        <div class="animateur-container">
            <h2>Mode Buzzer "Chacun pour sa peau !"<br><span>(Interface Animateur)</span></h2>
                    ...
                    if(confirm('Supprimer TOUS les participants ?')) {
                        ...
                        .then(data => {
                            if(data.success) {
                                ...
                                badButton.style.backgroundColor = '#8B0000';
                                badButton.style.color = '#fff';
                                // Forcer la déconnexion côté participant via localStorage
