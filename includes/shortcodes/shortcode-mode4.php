...
class InskillBuzzer_Shortcodes_Mode4 extends InskillBuzzer_Shortcodes {
    ...
    // Interface participant pour le quiz en équipe (Mode 4)
    public function render_mode4_participant($atts) {
        ...
        /* Espacement supplémentaire avant le classement */
        #scoreboard_mode4 {
            margin-top: 30px;
        }
        </style>
        ...
        <div class="participant-container">
            <h2 style="text-align:center;">
                Prêt à répondre ! (QCM)<br>
                Mode : "L'union fait la force"
            </h2>
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            var answerContainer = document.getElementById('quiz_buttons_mode4');
            ...
            // Mise à jour périodique du classement côté participant
            function updateRankingParticipant() {
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        document.getElementById('ranking_mode4').innerHTML = html;
                    }
                });
            }
            setInterval(updateRankingParticipant, 2000);

            // Réactivation des boutons réponses si aucun buzz (vérification du top 3)
            setInterval(function(){
                fetch(ajaxurl + '?action=get_top3_mode4')
                .then(response => response.json())
                .then(data => {
                    if(data.success) {
                        var top3 = data.data;
    ...
    // Interface animateur pour le quiz en équipe (Mode 4)
    public function render_mode4_animateur($atts) {
        ...
        /* Marges pour les tableaux */
        #team_scoreboard_mode4 {
            margin-top: 30px;
        }
        #participants_list_mode4 {
            margin-top: 30px;
        }
        </style>
        <div class="animateur-container">
            ...
            <div>
                ...
                <div class="result-row">
                    <button id="good_button_mode4" class="action-btn result-btn">Good !</button>
                    <label>Réponse OK : 
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            function attachBonusListeners() {
                var bonusButtons = document.querySelectorAll('.bonus-btn');
            ...
            function attachEjectListeners() {
                var ejectButtons = document.querySelectorAll('.eject-btn');
            ...
            if(resetButton) {
                resetButton.addEventListener('click', function(){
                    fetch(ajaxurl + '?action=reset_answers_mode4', { method: 'POST' })
                    .then(response => response.json())
                    .then(data => {
                        if(data.success) {
                            updateTop3();
