...
class InskillBuzzer_Shortcodes_Mode2 extends InskillBuzzer_Shortcodes {
    ...
    public function render_mode2_animateur($atts) {
        ...
        /* Correction de l'espacement entre le classement et le bloc Participants connectés */
        #participants_list_mode2 {
            margin-top: 20px;
        }
        </style>
            ...
            function updateParticipantsList() {
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        participantsList.innerHTML = '<h3 style="text-align:left;">Participants connectés</h3>' + html;
                        ...
                        if(confirm('Ejecter ' + participantName + ' ?')) {
                            ...
                            .then(data => {
                                if(data.success) {
                                    ...
                                    updateRanking();
                                    // Reprise de la logique du mode 1 : forcer la déconnexion côté participant
            ...
            setInterval(function(){
                ...
                .then(data => {
                    if(data.success) {
                        var participants = data.data;
                        var found = participants.some(function(item){
                            return item.participant_name === localStorage.getItem('currentParticipantMode2');
                        });
                        // Si non trouvé, forcer la déconnexion
                        if(!found && localStorage.getItem('currentParticipantMode2')) {
                            alert("Vous avez été déconnecté.");
                            window.location.reload();
                        }
                    }
