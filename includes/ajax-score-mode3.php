...
/**
 * Crée ou réinitialise un enregistrement pour un participant en Mode 3.
 */
...
/**
 * Enregistre un buzz pour le participant (met à jour buzz_time) en Mode 3.
 */
...
/**
 * Récupère les 3 premiers buzz pour le mode 3.
 */
function inskill_buzzer_get_top3_score_mode3() {
    ...
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT participant_name FROM {$wpdb->prefix}inskill_buzzer_mode3_scores WHERE activity_id = %d AND buzz_time IS NOT NULL ORDER BY buzz_time ASC LIMIT 3",
        $activity_id
    ), ARRAY_A );
...
/**
 * Met à jour le score du participant ayant buzzé en premier pour le mode 3.
 */
...
/**
 * Récupère le classement complet pour le mode 3.
 */
function inskill_buzzer_get_full_ranking_mode3() {
    ...
    $results = $wpdb->get_results( $wpdb->prepare(
        "SELECT participant_name, score FROM {$wpdb->prefix}inskill_buzzer_mode3_scores WHERE activity_id = %d ORDER BY score DESC, CASE WHEN buzz_time IS NULL THEN 1 ELSE 0 END, buzz_time ASC",
        $activity_id
    ), ARRAY_A );
    wp_send_json_success( $results );
}
...
/**
 * Réinitialise tous les buzzers pour le mode 3.
 */
function inskill_buzzer_reset_all_mode3() {
    ...
    $updated = $wpdb->query( $wpdb->prepare(
        "UPDATE {$wpdb->prefix}inskill_buzzer_mode3_scores SET buzz_time = NULL WHERE activity_id = %d",
        $activity_id
    ) );
    if ( false === $updated ) {
